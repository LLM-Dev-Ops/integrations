/**
 * Salesforce client core implementation following SPARC specification.
 *
 * Provides HTTP execution with authentication, rate limiting, circuit breaking,
 * and retry logic orchestration for Salesforce REST API and Bulk API.
 */

import { SalesforceConfig, SalesforceConfigBuilder } from '../config/index.js';
import {
  SalesforceError,
...
/**
 * Request options for Salesforce API calls.
 */
export interface RequestOptions {
  /** HTTP method */
  method: HttpMethod;
  /** Request path (relative to data API, e.g., '/sobjects/Account') */
  path: string;
  /** Query parameters */
  ...
  /** Return raw response (for CSV responses in Bulk API) */
  rawResponse?: boolean;
}
...
/**
 * Salesforce API client with resilience and observability.
 */
export class SalesforceClient {
  ...
  constructor(config: SalesforceConfig, observability?: Observability) {
    this.config = config;
   ...
   */
  get configuration(): SalesforceConfig {
    return this.config;
  }

  /**
   * Gets the base URL for API requests.
   * Returns instanceUrl + /services/data/apiVersion
   */
  get baseUrl(): string {
    return `${this.config.instanceUrl}/services/data/${this.config.apiVersion}`;
  }
      ...
      async (span) => {
        ...
        try {
          ...
          this.observability.metrics.increment(MetricNames.ERRORS_TOTAL, 1, {
            operation: options.method,
            error_type: (error as SalesforceError).code ?? 'unknown',
          });
  ...
  private async executeRequest<T>(options: RequestOptions): Promise<Response<T>> {
    ...
    const headers: Record<string, string> = {
      ...
      Accept: options.rawResponse ? 'text/csv' : 'application/json',
    ...
    try {
      ...
      if (!response.ok) {
        let errorBody: SalesforceApiErrorResponse | null = null;
        try {
          const contentType = responseHeaders['content-type'] ?? '';
          if (contentType.includes('application/json')) {
            errorBody = await response.json() as SalesforceApiErrorResponse;
          }
        } catch {
          // Ignore JSON parse errors
        }

        throw parseSalesforceApiError(
          response.status,
          errorBody,
      ...
      if (options.rawResponse) {
        // Return raw text for CSV responses (Bulk API)
...
/**
 * Creates a Salesforce client from a configuration.
 */
export function createSalesforceClient(config: SalesforceConfig, observability?: Observability): SalesforceClient {
  return new SalesforceClient(config, observability);
}

/**
 * Creates a Salesforce client from environment variables.
 */
export function createSalesforceClientFromEnv(observability?: Observability): SalesforceClient {
