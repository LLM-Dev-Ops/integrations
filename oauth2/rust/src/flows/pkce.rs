...
//!
//! RFC 6749 Section 4.1 with RFC 7636 PKCE extension.
...
use crate::core::{HttpMethod, HttpRequest, HttpTransport, PkceGenerator, StateManager};
use crate::error::{AuthorizationError, OAuth2Error};
use crate::types::{
    CallbackParams, ClientAuthMethod, CodeExchangeRequest, OAuth2Config,
    PkceAuthorizationParams, PkceAuthorizationUrl, PkceMethod, PkceParams, TokenResponse,
};
use crate::error::create_error_from_response;

/// PKCE Authorization Code Flow interface.
#[async_trait]
pub trait PkceAuthorizationCodeFlow: Send + Sync {
    /// Build authorization URL with PKCE challenge.
    fn build_authorization_url(&self, params: PkceAuthorizationParams) -> PkceAuthorizationUrl;
    ...
    /// Handle authorization callback with PKCE verification.
...
pub struct PkceAuthorizationCodeFlowImpl<T: HttpTransport, S: StateManager, P: PkceGenerator> {
    config: OAuth2Config,
    transport: Arc<T>,
    state_manager: Arc<S>,
...
impl<T: HttpTransport, S: StateManager, P: PkceGenerator> PkceAuthorizationCodeFlowImpl<T, S, P> {
    /// Create new PKCE Authorization Code Flow.
    pub fn new(
        config: OAuth2Config,
        transport: Arc<T>,
        state_manager: Arc<S>,
    ...
    fn build_auth_url_params(
        ...
        // PKCE parameters
        url_params.insert("code_challenge".to_string(), pkce.code_challenge.clone());
        url_params.insert(
            "code_challenge_method".to_string(),
            pkce.code_challenge_method.as_str().to_string(),
        );
    ...
    fn build_token_request_body(&self, request: &CodeExchangeRequest, code_verifier: &str) -> String {
        let mut params = vec![
        ...
        // Add client secret if using post method (optional for PKCE)
...
#[async_trait]
impl<T: HttpTransport, S: StateManager, P: PkceGenerator> PkceAuthorizationCodeFlow
    for PkceAuthorizationCodeFlowImpl<T, S, P>
{
    fn build_authorization_url(&self, params: PkceAuthorizationParams) -> PkceAuthorizationUrl {
        // Generate PKCE parameters
        let pkce_method = params.pkce_method.unwrap_or(PkceMethod::S256);
        let pkce = self.generate_pkce(pkce_method);

        let mut url_params = self.build_auth_url_params(&params, &pkce);
    ...
    async fn exchange_code(
        ...
        let body = self.build_token_request_body(&request, code_verifier);
        ...
        let token_response: TokenResponse = serde_json::from_str(&response.body).map_err(|e| {
            OAuth2Error::Protocol(crate::error::ProtocolError::InvalidJson {
                message: e.to_string(),
            })
        })?;
...
/// Mock PKCE Authorization Code Flow for testing.
#[derive(Default)]
pub struct MockPkceAuthorizationCodeFlow {
    build_url_history: std::sync::Mutex<Vec<PkceAuthorizationParams>>,
    exchange_history: std::sync::Mutex<Vec<(CodeExchangeRequest, String)>>,
    callback_history: std::sync::Mutex<Vec<(CallbackParams, String)>>,
    next_token_response: std::sync::Mutex<Option<TokenResponse>>,
    next_error: std::sync::Mutex<Option<OAuth2Error>>,
    next_pkce: std::sync::Mutex<Option<PkceParams>>,
}

impl MockPkceAuthorizationCodeFlow {
    ...
    /// Set next PKCE parameters.
    pub fn set_next_pkce(&self, pkce: PkceParams) -> &Self {
        *self.next_pkce.lock().unwrap() = Some(pkce);
        self
    }

    /// Get build URL history.
    pub fn get_build_url_history(&self) -> Vec<PkceAuthorizationParams> {
        self.build_url_history.lock().unwrap().clone()
    }

    /// Get exchange history.
    pub fn get_exchange_history(&self) -> Vec<(CodeExchangeRequest, String)> {
        self.exchange_history.lock().unwrap().clone()
    }

    /// Get callback history.
    pub fn get_callback_history(&self) -> Vec<(CallbackParams, String)> {
        self.callback_history.lock().unwrap().clone()
    }
}

#[async_trait]
impl PkceAuthorizationCodeFlow for MockPkceAuthorizationCodeFlow {
    fn build_authorization_url(&self, params: PkceAuthorizationParams) -> PkceAuthorizationUrl {
        self.build_url_history.lock().unwrap().push(params.clone());

        let pkce = self.next_pkce.lock().unwrap().take().unwrap_or(PkceParams {
            code_verifier: "mock-verifier-12345678901234567890123456789012".to_string(),
            code_challenge: "mock-challenge".to_string(),
            code_challenge_method: PkceMethod::S256,
        });

        PkceAuthorizationUrl {
            url: format!(
                "https://mock.example.com/authorize?redirect_uri={}",
                params.redirect_uri
            ),
    ...
    async fn exchange_code(
        ...
        self.exchange_history
            .lock()
            .unwrap()
            .push((request, code_verifier.to_string()));
    ...
    async fn handle_callback(
        ...
        self.callback_history
            .lock()
            .unwrap()
            .push((callback.clone(), code_verifier.to_string()));
...
/// Create mock PKCE Authorization Code Flow for testing.
pub fn create_mock_pkce_authorization_code_flow() -> MockPkceAuthorizationCodeFlow {
    MockPkceAuthorizationCodeFlow::new()
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_mock_pkce_flow() {
        let flow = MockPkceAuthorizationCodeFlow::new();
        ...
        assert!(!url.code_verifier.is_empty());
    ...
    #[tokio::test]
    async fn test_mock_exchange_with_pkce() {
        let flow = MockPkceAuthorizationCodeFlow::new();
        ...
        let response = flow
            .exchange_code(request, "test-verifier-12345678901234567890123456")
            .await
            .unwrap();
        assert_eq!(response.access_token, "mock-access-token");

        let history = flow.get_exchange_history();
        assert_eq!(history.len(), 1);
        assert_eq!(history[0].1, "test-verifier-12345678901234567890123456");
    }
}
