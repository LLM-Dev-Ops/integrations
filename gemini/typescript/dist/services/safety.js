 ...
 */
export function checkSafetyBlocks(response) {
 ...
 */
function checkPromptFeedback(feedback) {
    if (feedback.blockReason) {
        ...
        throw new SafetyBlockedError(category || feedback.blockReason, probability);
    }
}
/**
 ...
 */
function checkFinishReason(finishReason, safetyRatings) {
    switch (finishReason) {
        case 'SAFETY':
            {
                const category = extractPrimaryCategory(safetyRatings);
                const probability = extractPrimaryProbability(safetyRatings);
                throw new SafetyBlockedError(category || 'SAFETY', probability);
            }
        case 'RECITATION':
 ...
 */
function extractPrimaryCategory(ratings) {
 ...
 */
function extractPrimaryProbability(ratings) {
    if (!ratings || ratings.length === 0) {
        return undefined;
    }
    const priorityOrder = {
 ...
 */
export function hasSafetyConcerns(response) {
 ...
 */
export function getSafetyRatingSummary(response) {
    const summary = {
