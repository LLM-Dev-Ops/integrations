...
import { InvalidApiKeyError, ExpiredApiKeyError, AuthQuotaExceededError, ValidationError, InvalidModelError, InvalidParameterError, PayloadTooLargeError, UnsupportedMediaTypeError, TooManyRequestsError, QuotaExceededError, InternalServerError, ServiceUnavailableError, ModelOverloadedError, FileNotFoundError, ModelNotFoundError, CachedContentNotFoundError, SafetyBlockedError, RecitationBlockedError, ProhibitedContentError, DeserializationError, } from './categories.js';
/**
 * Maps HTTP status code to appropriate GeminiError
 */
export function mapHttpStatusToError(status, message, retryAfter) {
    switch (status) {
        case 400:
...
export function mapApiErrorToGeminiError(error) {
    ...
    if (code) {
        switch (code.toUpperCase()) {
            ...
            case 'SAFETY_BLOCKED':
                return new SafetyBlockedError(details?.category, details?.probability);
