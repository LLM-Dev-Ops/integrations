...
export class MockHttpClient {
     ...
     * ```
     */
    enqueueJsonResponse(status, body) {
     ...
     * ```
     */
    enqueueErrorResponse(status, message) {
        this.enqueueResponse({
            status,
     ...
     * ```
     */
    enqueueStreamingResponse(chunks) {
     ...
     */
    verifyRequest(index, method, urlContains) {
     ...
     */
    verifyHeader(index, headerName, headerValue) {
        if (index >= this.requests.length) {
            throw new Error(`No request at index ${index}`);
        }
        const request = this.requests[index];
        ...
        if (actualValue !== headerValue) {
            throw new Error(`Expected header '${headerName}' to be '${headerValue}', got '${actualValue}'`);
        }
    }
    /**
 ...
 * ```
 */
export function createMockFetch(mockClient) {
