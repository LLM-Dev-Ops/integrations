...
class TelemetryEmitter {
    ...
    async sendEvent(event) {
        try {
            const response = await (0, undici_1.request)(this.ingestUrl, {
                method: 'POST',
                headers: {
     ...
     */
    emitRequestStart(integration, correlationId, metadata = {}, options) {
        this.emit({
            correlationId,
            integration,
     ...
     */
    emitRequestComplete(integration, correlationId, metadata = {}, options) {
        this.emit({
            correlationId,
            integration,
            provider: options?.provider,
     ...
     */
    emitError(integration, correlationId, error, metadata = {}, options) {
        const errorMetadata = {
            ...metadata,
     ...
     */
    emitLatency(integration, correlationId, latencyMs, metadata = {}, options) {
