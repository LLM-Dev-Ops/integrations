/**
 * GCP Credentials Provider for Cloud Logging
 *
 * OAuth2 and service account authentication.
 * Reuses patterns from GCS credentials provider.
 */

import * as crypto from "crypto";
import * as fs from "fs/promises";
import type { GcpCredentials, ServiceAccountKey } from "../config/index.js";
...
export interface GcpAuthProvider {
  ...
  /**
   * Get the service account credentials (for signing).
   */
  getServiceAccountCredentials?(): Promise<ServiceAccountKey | undefined>;
}

/**
 * Cloud Logging API scope.
 */
const LOGGING_SCOPE = "https://www.googleapis.com/auth/logging.write https://www.googleapis.com/auth/logging.read";
...
export class ServiceAccountAuthProvider implements GcpAuthProvider {
  ...
  async getServiceAccountCredentials(): Promise<ServiceAccountKey> {
  ...
  private createJwt(): string {
    ...
    const signature = this.signRs256(
      signatureInput,
...
export class ApplicationDefaultAuthProvider implements GcpAuthProvider {
  ...
  async getServiceAccountCredentials(): Promise<ServiceAccountKey | undefined> {
    await this.ensureInitialized();
    return this.innerProvider?.getServiceAccountCredentials?.();
  }
  ...
  private async initialize(): Promise<void> {
    ...
    if (homeDir) {
      ...
      try {
        ...
        const creds = JSON.parse(content) as { type?: string } & ServiceAccountKey;
        ...
        if (creds.type === "authorized_user") {
          this.innerProvider = new UserCredentialsAuthProvider(
            creds as unknown as {
              client_id: string;
