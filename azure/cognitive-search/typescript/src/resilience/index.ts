/**
 * Azure Cognitive Search Resilience Module
 *
 * Provides retry logic and circuit breaker functionality for resilient operations.
 */

import type { RetryConfig, CircuitBreakerConfig } from '../client/config.js';
import { isRetryable, getRetryDelay } from '../errors/index.js';
  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    ...
    while (attempt < this.config.maxAttempts) {
      try {
        ...
        // Calculate delay - use retry-after from error if available
        const retryDelay = getRetryDelay(error);
        const delay = retryDelay ?? this.calculateDelay(attempt);
