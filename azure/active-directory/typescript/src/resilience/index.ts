/**
 * Azure Active Directory Resilience Module
 *
 * Provides retry logic and circuit breaker functionality for resilient OAuth2 operations.
 * Following the SPARC specification for Azure AD integration.
 */

import type { RetryConfig, CircuitBreakerConfig } from "../config.js";
import { AzureAdError, isRetryable } from "../error.js";
  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    ...
    while (attempt < this.config.maxAttempts) {
      try {
        ...
        if (error instanceof AzureAdError && !isRetryable(error)) {
          throw error;
        }
...
export class ResilientExecutor {
  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    ...
    if (!this.circuitBreaker.canExecute()) {
      throw new AzureAdError("SERVER_ERROR", "Circuit breaker is open - operation rejected", {
        isRetryable: false,
      });
    }
