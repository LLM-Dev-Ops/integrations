/**
 * Resilience components for Airtable client following SPARC specification.
 *
 ...
 * Implements per-base rate limiting for Airtable's 5 req/sec limit.
 */
export class RateLimiter {
   ...
   * @throws QueueTimeoutError if wait time exceeds queue timeout
   */
  async acquire(): Promise<void> {
    ...
    if (this.queue.length >= this.config.maxQueueSize) {
      throw new QueueTimeoutError(this.config.queueTimeout);
    }
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new QueueTimeoutError(this.config.queueTimeout));
    ...
    const wrappedOperation = async (): Promise<T> => {
      try {
        ...
        if (error instanceof RateLimitedError && !options.skipRateLimit) {
          this.rateLimiter.handleRateLimitResponse(error.retryAfter!);
        }
